...
-(BOOL) addDataAsFileToZip:(NSData *) data newname:(NSString*) newname fileDate:(NSDate *)fileDate
{
	if( !_zipFile )
		return NO;
	//	tm_zip filetime;
	...
	{	
		...
		zipInfo.dosDate= 0;
		zipInfo.tmz_date.tm_mday = [components day];
		zipInfo.tmz_date.tm_mon = [components month]-1;
		zipInfo.tmz_date.tm_year = [components year];
		zipInfo.tmz_date.tm_sec = [components second];
		zipInfo.tmz_date.tm_min = [components minute];
...
-(NSArray *) UnzipFileToDataArray
{
	...
	do{
		...
		NSMutableData *tmpData = [[NSMutableData alloc] init];
			...
			while( (read=unzReadCurrentFile(_unzFile, buffer, 4096)) > 0 )
			{
				[tmpData appendBytes:buffer length:read];
			}
		
		
		[retMut addObject:[NSData dataWithData:tmpData]];
		[tmpData release];
	...
	NSArray *retArray = [NSArray arrayWithArray:retMut];
	[retMut release];
	return retArray;
}
